resource "aws_cloudwatch_event_rule" "every_10_minutes_lambda" {
...
resource "aws_cloudwatch_event_target" "event_target" {
  rule      = "${aws_cloudwatch_event_rule.every_10_minutes_lambda.name}"
...
resource "aws_lambda_permission" "cloudwatch_permission" {
  ...
  source_arn    = "${aws_cloudwatch_event_rule.every_10_minutes_lambda.arn}"
}
